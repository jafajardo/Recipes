...
class RecipesList extends Component {
  ...
  retrieveFeaturedRecipes() {
    let featuredRecipes = [];
    console.log('Retrieve featured recipes: ', this.props.all_recipes);
    console.log(this.props.all_recipes.data);
  ...
  renderRecipesColumns() {
    ...
    return featuredRecipes.map((recipe, index) => {
      if (recipe) {
        return (
          <Link to={ `recipes/${ recipe._id }` } key={ `${recipe._id}idx${index}` }>
